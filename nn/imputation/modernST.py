...
# Import the layers from the previous file
from .modernST_layer import BackboneBlockImpute


class ModernSTImpute(nn.Module):
    """ModernST for spatiotemporal imputation task"""
    
    return_type = list  # Required for imputation models
    ...
    def __init__(self,
                 ...
                 ff_size=256,
                 n_nodes=None,
                 kernel_sizes=[5, 3],  # List of kernel sizes for each block
                 spatial_step=2,
                 patch_size=(100, 2),
                 rw_length=5,
                 rw_samples=100,
        ...
        # Calculate total features: node + exog features (edge/triangle handled dynamically)
        ...
        if self.use_learned_adj and n_nodes is not None:
            self.source_embeddings = NodeEmbedding(n_nodes, hidden_size)
            self.target_embeddings = NodeEmbedding(n_nodes, hidden_size)
        
        # Stem network for initial feature processing - uses input_size only (not total_features)
        ...
        if n_nodes is not None:
            self.layer_norm = nn.LayerNorm([n_nodes, hidden_size])
        
        # Backbone blocks
        self.blocks = nn.ModuleList([
            BackboneBlockImpute(n_nodes, kernel_size, spatial_step, self.total_features, 
                               hidden_size, activation, dropout)
            for kernel_size in kernel_sizes
        ])
        
        # Imputation head - maps to full sequence
    ...
    def _gather_walk_masks(self, masks, walks):
        """Gather masks along random walk paths"""
        # masks: (batch, time, nodes, features)
        # walks: (batch, samples, time, nodes, length)
        
        masks = rearrange(masks, 'b t n f -> b 1 t n 1 f')
        walks = rearrange(walks, 'b s t n l -> b s t n l 1')
        
        # Gather masks for each walk step
        gathered = torch.gather(
            masks.expand(-1, walks.shape[1], -1, -1, walks.shape[4], -1),
            3,
            walks.expand(-1, -1, -1, -1, -1, masks.shape[5])
        )
        
        # Output: (batch, samples, time, nodes, length, features)
        return gathered

    def _prepare_feature_and_mask_lists(self, x, mask, edge_features, triangle_features, 
                                       batch_size, time_steps, num_nodes):
        """Prepare feature and mask lists for concatenation"""
        
        # Features list: node + edge + triangle features (concatenated along feature dimension)
        features_list = [x]
        
        # Masks list: node mask + edge indicator + triangle indicator
        masks_list = [mask]
        
        if edge_features is not None:
            # edge_features should be (num_nodes, edge_feature_size) 
            # Expand to (batch, time, nodes, edge_feature_size)
            if edge_features.dim() == 1:
                edge_features = edge_features.unsqueeze(-1)
            edge_features_expanded = edge_features.unsqueeze(0).unsqueeze(0).repeat(
                batch_size, time_steps, 1, 1
            )
            features_list.append(edge_features_expanded)
            
            # Create indicator tensor for edge features (all ones - no missing values)
            edge_indicator = torch.ones_like(edge_features_expanded, device=x.device, dtype=mask.dtype)
            masks_list.append(edge_indicator)
            
            
            
        if triangle_features is not None:
            # triangle_features should be (num_nodes, triangle_feature_size)
            # Expand to (batch, time, nodes, triangle_feature_size)
            if triangle_features.dim() == 1:
                triangle_features = triangle_features.unsqueeze(-1)
            triangle_features_expanded = triangle_features.unsqueeze(0).unsqueeze(0).repeat(
                batch_size, time_steps, 1, 1
            )
            features_list.append(triangle_features_expanded)
            
            # Create indicator tensor for triangle features (all ones - no missing values)
            triangle_indicator = torch.ones_like(triangle_features_expanded, device=x.device, dtype=mask.dtype)
            masks_list.append(triangle_indicator)
        
        # Concatenate along element dimension
        x_concat = torch.cat(features_list, dim=-2)
        mask_concat = torch.cat(masks_list, dim=-2)
    ...
    def get_learned_adjacency(self):
        ...
        if not self.use_learned_adj:
            return None
        logits = F.gelu(self.source_embeddings() @ self.target_embeddings().T)
        return torch.softmax(logits, dim=1)

    def forward(self, x, edge_index, edge_weight, mask=None, u=None,
        ...
        Args:
            ...
            edge_features: Edge-level features (num_nodes, edge_feature_size)
            triangle_features: Triangle-level features (num_nodes, triangle_feature_size)
        
        Returns:
            [imputation]: List containing imputation results for compatibility with TSL
        """
        ...
        # Prepare features and masks (node + edge + triangle)
        x_concat, mask_concat = self._prepare_feature_and_mask_lists(
            x, mask, edge_features, triangle_features,
            batch_size, time_steps, num_nodes
        )
        ...
        x_walks = torch.cat([gathered_features, gathered_masks], dim=-1)
        
        # Add uniqueness features
        x_walks = torch.cat([x_walks, uniqueness_features], dim=-1)
        x_walks = x_walks.unsqueeze(-2)  # Add dimension for stem processing
        
        # Process exogenous features (after random walk gathering, like forecasting)
        if u is not None:
        ...
        # Generate imputation for ALL timesteps
        x_impute = rearrange(x_final, 'b t n d f -> b t n (f d)')
        imputation = self.head(x_impute)  # (batch, time, nodes, input_size)
        
        # Return as list for compatibility with TSL imputation framework
        return imputation

    def predict(self, x, edge_index, edge_weight, mask=None, u=None,
                rw_edge_index=None, rw_edge_weight=None, 
                edge_features=None, triangle_features=None):
        """Prediction method for inference"""
        return self.forward(
            x=x, edge_index=edge_index, edge_weight=edge_weight,
            mask=mask, u=u, rw_edge_index=rw_edge_index,
            rw_edge_weight=rw_edge_weight, edge_features=edge_features,
